angular.module('searchLagNodplakatPanel').controller('searchLagNodplakatPanelController', [
  '$scope',
  'mainAppService',
  '$http',
  'ISY.MapAPI.Map',
    ...
    var _retrieveDataFromResponse = function (name, data) {
      ...
      case 'placenamesByBbox':
        $scope.lagNodplakatDict[name] = data.navn.map(function (a) {
          a.skrivemaatenavn = a.stedsnavn[0]['skrivem\xe5te'];
          // + ' ' + a.meterFraPunkt + 'm fra søkt punkt'
          return a;
        });
        if (!$scope.activePlaceName) {
          if (data.navn.length > 0) {
            $scope.activePlaceName = data.navn[0].stedsnavn[0]['skrivem\xe5te'];
            $scope.setSearchBarText($scope.activePlaceName);
            $scope.lagNodplakatName = $scope.activePlaceName;
          }
        }
