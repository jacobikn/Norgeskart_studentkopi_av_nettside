angular.module('searchKoordTransPanel').controller('searchKoordTransPanelController', [
  '$scope',
  'mainAppService',
  '$http',
  'searchKoordTransPanelFactory',
  function ($scope, mainAppService, $http, searchKoordTransPanelFactory) {
    ...
    var decToDMS = function (deg, format, dp) {
      ...
      case 'deg+min':
        d = Math.floor(deg);
        // get component deg
        m = _round(deg * 60 % 60, dp);
        // get component min & round
        if (m == 60) {
          m = 0;
          d++;
        }
        // check for rounding up
        dms = sign + d + '\xb0' + DmsSeparator + m + '\u2032';
        break;
      case 'dms':
      case 'deg+min+sec':
        d = Math.floor(deg);
        ...
        // get component min
        s = _round(deg * 3600 % 60, dp);
        // get component sec & round
        if (s == 60) {
          s = _round(0, dp);
          m++;
        }
        ...
        // check for rounding up
        dms = sign + d + '\xb0' + DmsSeparator + m + '\u2032' + DmsSeparator + s + '\u2033';
        break;
      }
      return dms;
    };
